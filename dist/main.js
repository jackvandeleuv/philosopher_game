...
let canvas1 = document.getElementById('gameCanvas1');
let ctx1 = canvas1.getContext('2d');
let canvas2 = document.getElementById('gameCanvas2');
let ctx2 = canvas2.getContext('2d');
let game = new Game(new Player('Player1'), new Player('Player2'), philosophers.slice(0, 3), philosophers.slice(3));
let manager1 = new StateManager(ctx1, game, 0, imageRepo1);
let manager2 = new StateManager(ctx2, game, 1, imageRepo2);
function gameLoop(manager1, manager2, game) {
    const gameLoopStep = () => {
        ...
        if (nextScene1 != null) {
            manager1.pushGameScene(nextScene1);
        }
        if (nextScene2 != null) {
            manager2.pushGameScene(nextScene2);
        }
        // Get next menu state from the game logics
        let nextState1 = game.getNextMenuState1();
        let nextState2 = game.getNextMenuState2();
        if (nextState1 != null) {
            manager1.changeMenuState(nextState1);
        }
        if (nextState2 != null) {
            manager2.changeMenuState(nextState2);
        }
